...
test.describe("Generate Screenshots", () => {
  ...
  test("capture settings", async ({ page }) => {
    ...
    await expect(page.getByText(/Manage your credentials/i)).toBeVisible({
      timeout: 10000,
    });
  ...
  test("capture workspace view", async ({ page }) => {
    ...
    const openBtn = page
      .locator("button")
      .filter({ hasText: /Open Box/i })
      .first();
    if (await openBtn.isVisible()) {
      await openBtn.click();
      await expect(
        page.getByText(/Back to Dashboard/i).or(page.getByText(/Box Details/i)),
      ).toBeVisible({ timeout: 10000 });
      await page.waitForTimeout(2000);
