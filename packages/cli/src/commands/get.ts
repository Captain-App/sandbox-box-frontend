...
export const getCommand = new Command('get')
  ...
  .action(async (runId, options) => {
    try {
      ...
      console.log(`${chalk.bold('Status:')} ${session.status === 'completed' ? chalk.green(session.status) : (session.status === 'failed' ? chalk.red(session.status) : chalk.yellow(session.status))}`);
      console.log(`${chalk.bold('Task:')} ${session.task || 'N/A'}`);
      console.log(`${chalk.bold('Created:')} ${new Date(session.createdAt * 1000).toLocaleString()}`);
