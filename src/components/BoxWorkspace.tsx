...
import { Send, Paperclip, Bot, User, ExternalLink, Maximize2, Minimize2, ChevronLeft, Loader2, Sparkles, Terminal, FileEdit, Code2, AlertCircle } from "lucide-react"
...
export function AgentMonologue({ sandbox, plan, isWorking, currentRunId, events, onSend }: AgentMonologueProps) {
  ...
  const renderEvent = (event: RealtimeEvent) => {
    switch (event.type) {
      case "step-start":
        return (
          <div key={event.seq} className="flex items-start gap-2 mb-3 animate-in fade-in slide-in-from-left-2 duration-300">
            <div className="mt-1 p-1 rounded bg-blue-500/10 border border-blue-500/20">
              <Sparkles className="w-3 h-3 text-blue-400" />
            </div>
            <div className="flex-1">
              <div className="text-[10px] font-black uppercase tracking-widest text-blue-400/70">Thinking</div>
              <div className="text-xs text-blue-100/80">{event.data.name || "Considering next steps..."}</div>
            </div>
          </div>
        )
      case "command.executed":
        return (
          <div key={event.seq} className="flex items-start gap-2 mb-3 animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <div className="flex-1">
              <div className="text-[10px] font-black uppercase tracking-widest text-green-400/70">Command</div>
      ...
      case "file.edited":
        return (
          <div key={event.seq} className="flex items-start gap-2 mb-3 animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <div className="flex-1">
              <div className="text-[10px] font-black uppercase tracking-widest text-amber-400/70">Edit</div>
              <div className="text-xs text-amber-100/80">Modified <span className="font-mono text-[10px] bg-white/5 px-1 rounded">{event.data.path}</span></div>
            </div>
          </div>
        )
      case "tool":
        return (
          <div key={event.seq} className="flex items-start gap-2 mb-3 animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <div className="flex-1">
              <div className="text-[10px] font-black uppercase tracking-widest text-purple-400/70">Tool Call</div>
              <div className="text-xs text-purple-100/80">Using <span className="font-mono text-[10px] bg-white/5 px-1 rounded">{event.data.name}</span></div>
            </div>
          </div>
        )
      case "session.error":
        return (
          <div key={event.seq} className="flex items-start gap-2 mb-3 animate-in fade-in slide-in-from-left-2 duration-300">
            ...
            <div className="flex-1">
              <div className="text-[10px] font-black uppercase tracking-widest text-red-400/70">Error</div>
              <div className="text-xs text-red-400/90">{event.data.message || "An unexpected error occurred"}</div>
            </div>
          </div>
        )
    ...
    <div className="flex flex-col h-full bg-slate-950 border-r border-white/5">
      ...
      <div className="p-4 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-bold truncate max-w-[120px]">{sandbox.title || sandbox.id}</div>
            <div className="text-[10px] text-muted-foreground uppercase tracking-widest">Agent Chat</div>
          </div>
        </div>
        {isWorking && (
          <div className="flex items-center gap-2 px-2 py-1 rounded-full bg-primary/10 border border-primary/20">
            <Loader2 className="w-3 h-3 text-primary animate-spin" />
            <span className="text-[10px] font-black uppercase tracking-widest text-primary">Working</span>
          </div>
        )}
      </div>
...
export function BoxWorkspace({ sandbox, onClose }: BoxWorkspaceProps) {
  ...
  const [activeRightView, setActiveRightView] = useState<'preview' | 'plan'>('preview')
  const [activeMobileTab, setActiveMobileTab] = useState<'chat' | 'preview' | 'plan'>('chat')
  ...
  // WebSocket hook
  const { events, clearEvents } = useRealtime(sessionId, sandbox.realtimeToken || null)
      ...
      <div className="h-12 md:h-14 bg-slate-950 border-b border-white/5 flex items-center justify-between px-2 md:px-4">
        ...
        <div className="hidden md:flex items-center bg-white/5 rounded-lg p-1">
          <button
            ...
            className={cn(
              ...
              activeRightView === 'preview' ? "bg-primary text-white" : "text-muted-foreground hover:text-white/70"
            )}
          >
            Preview
          </button>
          <button
            ...
            className={cn(
              "px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-md transition-all",
              activeRightView === 'plan' ? "bg-primary text-white" : "text-muted-foreground hover:text-white/70"
            )}
          >
            Plan
          </button>
        ...
        <div className="flex items-center bg-white/5 rounded-lg p-1 md:hidden">
          ...
          <button
            ...
            className={cn(
              "px-2 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all",
              activeMobileTab === 'plan' ? "bg-primary text-white" : "text-muted-foreground"
            )}
          >
            Plan
          </button>
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          >
            ...
            <span className="hidden lg:inline text-[10px] font-bold uppercase tracking-widest">Open Tab</span>
          </Button>
          <button 
        ...
        <div className="flex-1 relative bg-slate-950">
            ...
            <div className={cn(
              ...
              <div className="max-w-4xl mx-auto">
                <div className="prose prose-invert prose-sm lg:prose-base max-w-none prose-headings:tracking-tight prose-headings:uppercase prose-headings:text-primary prose-a:text-primary hover:prose-a:underline prose-code:text-primary prose-code:bg-primary/10 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-white/5 prose-pre:border prose-pre:border-white/10">
                  <ReactMarkdown>{plan || "# No Plan Found\nYour agent hasn't created a plan for this session yet."}</ReactMarkdown>
                </div>
              </div>
            </div>
          )}
          ...
          {(activeRightView === 'preview' || activeMobileTab === 'preview') && (
            <div className={cn(
              ...
              <iframe 
                src={iframeUrl}
                className={cn("w-full h-full border-0", previewLoaded ? "z-10 relative" : "")}
