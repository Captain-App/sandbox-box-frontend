...
describe("SecretsVault Component", () => {
  ...
  it("shows empty state when no secrets", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/No secrets found in your vault/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("toggles add secret form", async () => {
    ...
    expect(
      screen.getByPlaceholderText("e.g. GITHUB_TOKEN"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByPlaceholderText("e.g. GITHUB_TOKEN"),
    ).not.toBeInTheDocument();
  });

  it("can create a new secret", async () => {
    vi.mocked(api.getBoxSecrets).mockResolvedValue([]);
    vi.mocked(api.createBoxSecret).mockResolvedValue({
      id: "3",
      name: "NEW_SECRET",
      hint: "1234",
      createdAt: Date.now() / 1000,
    ...
    fireEvent.change(screen.getByPlaceholderText("e.g. GITHUB_TOKEN"), {
      target: { value: "NEW_SECRET" },
    });
    fireEvent.change(screen.getByPlaceholderText("Paste value here"), {
    ...
    await waitFor(() => {
      expect(api.createBoxSecret).toHaveBeenCalledWith(
        "NEW_SECRET",
        "secret-value",
      );
