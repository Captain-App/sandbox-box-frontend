...
export function BillingDashboard() {
  ...
  const balanceAmount = balance
    ? (balance.balanceCredits / 100).toFixed(2)
    : "0.00";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Credits-first metering. No surprises, no runaway costs.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 md:p-8 rounded-[2rem] border border-white/5 bg-white/5 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Approx. {balance?.balanceCredits || 0} minutes of compute remaining.
          </p>
        </div>
        ...
        <div className="p-6 md:p-8 rounded-[2rem] border border-white/5 bg-white/5 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Total spent this month across all agent activity.
          </p>
        </div>
        ...
        <div className="p-6 md:p-8 rounded-[2rem] border border-white/5 bg-white/5 space-y-4">
          ...
          <div className="text-3xl md:text-4xl font-black tracking-tighter text-muted-foreground/50">
            --%
          </div>
          <p className="text-xs text-muted-foreground">
            Efficiency metrics coming soon with session analytics.
          </p>
        </div>
      </div>
      ...
      {balance && balance.balanceCredits < 2500 && (
        <div className="p-6 rounded-3xl bg-amber-500/5 border border-amber-500/20 flex flex-col sm:flex-row sm:items-center gap-4 animate-in fade-in slide-in-from-top-4">
          ...
          <p className="text-sm text-amber-200/80">
            Your balance is below £25. Enable auto-recharge to ensure your agent
            doesn't pause during long-running tasks.
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
              ...
              <tbody className="divide-y divide-white/5">
                {transactions.map((tx) => (
                  ...
                  >
                    ...
                    >
                      {tx.amountCredits > 0 ? "+" : ""}£
                      {(tx.amountCredits / 100).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="md:hidden space-y-3">
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground text-sm border border-white/5 rounded-3xl bg-white/5">
              No transactions found.
            </div>
          ) : (
            transactions.map((tx) => (
              <div
                key={tx.id}
                className="p-4 rounded-2xl border border-white/5 bg-white/5 flex flex-col gap-3"
              >
