...
import { 
  Shield, CreditCard, Box, 
  ArrowLeft, Github, Key, Lock, CheckCircle2, 
...
export function UserPage() {
  ...
  if (loading) return <div className="animate-pulse py-20 text-center">Loading User Data...</div>
  if (!user) return <div className="py-20 text-center text-red-500">User Not Found</div>
      ...
      <div className="flex items-start justify-between">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black tracking-tighter uppercase italic">{user.email || 'Anonymous User'}</h1>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="px-4 py-2 rounded-2xl bg-slate-900 border border-white/5 text-right">
            ...
            <p className="text-2xl font-black tracking-tight">£{(user.balanceCredits / 100).toFixed(2)}</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">GitHub</p>
                    <p className="text-sm font-bold text-white">{user.githubInstallation?.accountLogin || 'Not Linked'}</p>
                  </div>
                </div>
              ...
              <div className="p-4 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Anthropic Key</p>
          ...
          <div className="p-8 rounded-3xl border border-white/5 bg-slate-900/50 backdrop-blur-xl">
            ...
            <div className="space-y-2">
              {transactions.map((tx: any) => (
                <div key={tx.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                  <div className={cn(
                    "w-8 h-8 rounded-lg flex items-center justify-center",
                    tx.amount_credits > 0 ? "bg-green-500/10 text-green-500" : "bg-blue-500/10 text-blue-500"
                  )}>
                    {tx.amount_credits > 0 ? <ArrowDownRight className="w-4 h-4" /> : <ArrowUpRight className="w-4 h-4" />}
                  </div>
                  ...
                  <div className={cn(
                    ...
                    {tx.amount_credits > 0 ? '+' : ''}£{(tx.amount_credits / 100).toFixed(2)}
                  </div>
                </div>
              ))}
              {transactions.length === 0 && <p className="text-center py-8 text-slate-500 uppercase font-black text-xs tracking-widest">No transactions found</p>}
            </div>
          </div>
        </div>
          ...
          <div className="p-8 rounded-3xl border border-white/5 bg-slate-900/50 backdrop-blur-xl">
            ...
            <div className="space-y-3">
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white group-hover:text-primary transition-colors">{sb.title || sb.sessionId}</p>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                          Created {new Date(sb.createdAt * 1000).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className={cn(
                      ...
                      sb.status === 'active' ? "bg-green-500/10 border-green-500/20 text-green-500" : "bg-white/5 border-white/10 text-slate-500"
                    )}>
                      {sb.status || 'unknown'}
                    </div>
                  </div>
              ...
              {(!user.sessions || user.sessions.length === 0) && (
                <p className="text-center py-8 text-slate-500 uppercase font-black text-xs tracking-widest">No sessions found</p>
              )}
            </div>
          </div>
          ...
          <div className="p-8 rounded-3xl border border-white/5 bg-slate-900/50 backdrop-blur-xl">
            ...
            <div className="space-y-2">
              {user.boxSecrets?.map((secret: any) => (
                <div key={secret.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
              ...
              {user.boxSecrets?.length === 0 && <p className="text-xs text-slate-500 italic">No secrets configured</p>}
            </div>
          </div>
        </div>
      </div>
