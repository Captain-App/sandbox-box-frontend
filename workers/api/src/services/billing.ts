...
export interface BillingServiceInterface {
  readonly getBalance: (
    userId: string,
  ) => Effect.Effect<UserBalance, SessionStorageError>;
  ...
  readonly getStripeCustomerId: (
    userId: string,
  ) => Effect.Effect<string | null, SessionStorageError>;
  readonly setStripeCustomerId: (
    userId: string,
    customerId: string,
  ) => Effect.Effect<void, SessionStorageError>;
  readonly grantStarterCredits: (
    userId: string,
  ) => Effect.Effect<boolean, SessionStorageError>;
}
    ...
    reportTokenUsage: (
      ...
      Effect.gen(function* () {
        ...
        const credits = Math.ceil(
          (inputTokens / 1000) * 1 + (outputTokens / 1000) * 5,
        );
        if (credits <= 0) return;
            ...
            await db.batch([
              db
                ...
                .bind(
                  ...
                  amount,
                  "usage",
                  `AI token usage for ${sessionId} (${model})`,
                  now,
                  JSON.stringify({
                    sessionId,
                    service,
      ...
      Effect.tryPromise({
        try: async () => {
          ...
          await db
            ...
            .bind(userId, now, customerId, customerId, now)
            .run();
        },
        catch: (error) =>
          new SessionStorageError({
...
export function makeBillingServiceLayer(
  db: D1Database,
