...
export interface HoneycombServiceInterface {
  readonly listRecentTraces: (
    limit: number,
  ) => Effect.Effect<HoneycombSpan[], SessionStorageError>;
  readonly getTraceDetails: (
    traceId: string,
  ) => Effect.Effect<HoneycombTrace, SessionStorageError>;
  readonly getSessionTraces: (
    sessionId: string,
  ) => Effect.Effect<HoneycombSpan[], SessionStorageError>;
}
      ...
      Effect.tryPromise({
        try: async () => {
          const query = {
            ...
            filters: [{ column: "trace.parent_id", op: "does-not-exist" }],
            limit,
...
export function makeHoneycombServiceLayer(
  apiKey: string,
  dataset: string,
): Layer.Layer<HoneycombService> {
